...
import { ensureSepolia } from "@/lib/ensure-sepolia";
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const contract = new ethers.Contract(
        ...
        ["function issuePrescription(uint256 slotId, string memory _medicines, string memory _notes) public"],
        signer
      );

      const numericSlotId = parseInt(id.slice(-8), 16);
      const stringifiedMedicines = JSON.stringify(medicines);
      
      const tx = await contract.issuePrescription(numericSlotId, stringifiedMedicines, notes);
      toast.info("Saving prescription on-chain... please wait.");
      ...
      if (res.ok) {
        toast.success(
          <div className="flex flex-col gap-1">
            ...
            <a href={`https://sepolia.etherscan.io/tx/${txHash}`} target="_blank" className="text-xs font-bold underline">
              View on Etherscan
            </a>
          </div>
        );
      } else {
        const data = await res.json().catch(() => ({ error: "Payload too large or server error" }));
        toast.error(data.error || "Failed to save prescription.");
      }
    } catch (error: any) {
      console.error(error);
      const msg = error?.reason || error?.message || "Failed to save.";
