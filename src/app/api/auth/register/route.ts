...
export async function POST(req: Request) {
  try {
    ...
    if (role === "DOCTOR") {
      ...
      // Check if admin already registered this wallet (case-insensitive match)
      const normalizedWallet = walletAddress.toLowerCase();
      const existingDoctor = await User.findOne({ walletAddress: { $regex: new RegExp(`^${normalizedWallet}$`, 'i') }, role: "DOCTOR" });
      if (!existingDoctor) {
