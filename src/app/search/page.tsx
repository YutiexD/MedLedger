...
export default function SearchPage() {
  ...
  const handleBook = async (slotId: string, doctorWallet: string) => {
    ...
    try {
      // 0. Ensure we're on Sepolia testnet
      const onSepolia = await ensureSepolia();
      if (!onSepolia) {
        toast.error("Please switch to Sepolia testnet in MetaMask to continue.");
        return;
      }

      // 1. Trigger On-Chain Transaction (uses Sepolia ETH only)
      ...
      // For this demo we'll use the last 8 chars of the mongoId which contains the incrementing counter
