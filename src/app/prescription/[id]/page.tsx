...
import { Printer, User, ClipboardList, ShieldCheck } from "lucide-react";

export default function PrescriptionPrintPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    const fetchPrescription = async () => {
      try {
        const res = await fetch(`/api/prescriptions?slotId=${id}`);
        ...
        setPrescription({
          ...
          date: new Date().toLocaleDateString(),
          medicines: [],
          notes: "Unable to load prescription data.",
        });
      }
    };
    fetchPrescription();
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 p-8 sm:p-16 max-w-4xl mx-auto pt-20">
      {/* Header / Brand */}
      ...
      <div className="grid grid-cols-2 gap-12 mb-12">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xl font-bold">{prescription.doctor?.name || "Doctor"}</div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xl font-bold">{prescription.patient?.name || "Patient"}</div>
      ...
      {prescription.attachmentBase64 && (
        <div className="mb-12">
          <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-400 mb-4 flex items-center gap-2">
            <ClipboardList className="w-4 h-4" /> Attached Document / Report
          </h3>
          <div className="border-4 border-zinc-900 rounded-3xl overflow-hidden shadow-[8px_8px_0px_0px_rgba(24,24,27,1)]">
            {prescription.attachmentType?.includes("pdf") ? (
              <iframe 
                src={prescription.attachmentBase64} 
              ...
              <img 
                ...
                className="w-full h-auto max-h-[800px] object-contain bg-zinc-50"
              />
            )}
          </div>
        </div>
      ...
      <div className="mt-20 pt-10 border-t border-zinc-100 text-center space-y-4">
        {prescription.txHash && (
          <div className="mb-6 flex justify-center">
            <a
              href={`https://sepolia.etherscan.io/tx/${prescription.txHash}`}
              target="_blank"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl text-sm font-bold bg-indigo-50 text-indigo-600 border-2 border-indigo-200 shadow-[4px_4px_0px_0px_rgba(79,70,229,1)] hover:shadow-none hover:translate-y-[2px] hover:translate-x-[2px] transition-all"
            >
              <ShieldCheck className="w-5 h-5" /> Verify On-Chain Medical Record
            </a>
          </div>
        )}
        <div className="text-xs font-bold text-zinc-300 uppercase tracking-[0.2em]">Digital Document ID: {prescription.id || id}</div>
